...
const segmentBase = cva('inline-flex box-border relative h-10 p-1 bg-neutral-900 rounded-full', {
  variants: {
    size: {
      sm: 'h-9',
      md: 'h-10',
...
const segmentItem = cva(
  `h-full flex gap-0.5 items-center justify-center select-none z-2 transition rounded disabled:cursor-not-allowed disabled:opacity-50 outline-none px-3`,
  {
    variants: {
      size: {
        sm: 'text-sm',
...
const segmentIndicator = cva(
  `h-full transition ease-linear box-border absolute top-0 shadow outline-none bg-neutral-800 disabled:cursor-not-allowed rounded-full
...
export default function Segment({
  ...
  return (
    <div ref={ref} className={cn(segmentBase({ size, className }))} {...props}>
      <div
        ref={segmentRef}
        className={cn('relative flex w-full', { grid: equaledWidth })}
        ...
        {options.length > 0 && (
          <div
            ref={indicatorRef}
            data-segment-item="segment-indicator"
